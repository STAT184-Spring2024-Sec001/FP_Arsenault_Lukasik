...
From here, we can continue to look at other statistics and how they affect the outcome in the race. The next visualization will look at qualifying time and how often qualifying each first, second, third, fourth, and fifth generates a race win. We will make a bar graph to display the amount of times the qualifying positions of 1st - 5th actually won the race.
...
# Count the number of race wins for each qualifying position
win_counts <- qualifying_winners %>%
  group_by(position.x) %>%
  summarize(race_wins = n())

# Make a bar graph using ggplot
ggplot(win_counts, aes(x = factor(position.x), y = race_wins)) +
  geom_bar(stat = "identity", fill = "skyblue") +
  labs(x = "Qualifying Position", y = "Number of Race Wins", 
       title = "Race Wins by Qualifying Position (1st to 5th)") +
  theme_minimal() +
