...
library(knitr)
```
>>>>>>> 473eaf87b7c7b430a99ba18eaa634d683bbe635a
...
# Assuming 'pit_stops' and 'results' datasets are already loaded

# Merge 'pit_stops' with 'results' to get race outcomes
merged_data <- merge(pit_stops, results, by = "raceId")

# Filter data for completed races (exclude races where status is not 'Finished')
completed_races <- merged_data[merged_data$statusId == 1, ]

# Check the column names of the merged dataset
colnames(completed_races)

# Create a scatter plot to visualize the relationship between pit stop time and race position
ggplot(completed_races, aes(x = milliseconds, y = positionOrder)) +
  geom_point(alpha = 0.6) +
  labs(x = "Pit Stop Time (milliseconds)", y = "Race Position") +
  ggtitle("Relationship Between Pit Stop Time and Race Position") +
  theme_minimal()
